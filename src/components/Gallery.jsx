...
const Gallery = () => {
  ...
  if (!selectedCategory) {
    return <div className="error-message text-center py-22"><div className="text-center py-6 text-3xl font-bold text-[#502D18]">Category not found</div> <button 
        onClick={() => navigate(-1)} 
        className="mb-4 text-[#502D18] px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-lg "
      >
        ← Back to Events
      </button></div>;
  }
      ...
      <div className="image-grid gallery">
        {selectedCategory.images.map((item) => (
          ...
          >
            <img
              src={item.imgSrc}
              alt={item.imgTitle || `${selectedCategory.name1} image ${item.id}`}
              className="gallery-image w-full p-2 rounded-3xl"
              onError={(e) => {
